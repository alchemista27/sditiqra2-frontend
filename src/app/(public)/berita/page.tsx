    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '6rem 1.5rem 3rem' }}>
      ...
      {posts.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: '#9CA3AF' }}>
          <span className="material-symbols-outlined" style={{ fontSize: 64, color: '#D1D5DB', marginBottom: '1rem' }}>newspaper</span>
          ...
          {posts.map((post: any) => (
            <Link key={post.id} href={`/berita/${post.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
              <article style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #E5E7EB' }} className="hover-card">
                <div style={{ height: 180, background: 'linear-gradient(135deg, #1B6B44, #2D9164)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {post.coverImage ? <img src={`${API_BASE}${post.coverImage}`} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span className="material-symbols-outlined" style={{ fontSize: 48, color: 'rgba(255,255,255,0.8)' }}>newspaper</span>}
                </div>
